...
export default class OrderController {
  ...
  public getCustomerOrders = async ({ customerId }: { customerId: string }) => {
    try {
  ...
  public checkout = async ({
    ...
    try {
      ...
      const order = await Order.create({
        ...
        deliveryStatus: reservePickup == "true" ? "pending" : "delivered",
        totalPrice: totalPrice + serviceTotalPrice - parseFloat(discount),
  ...
  public editOrderItem = async ({
    ...
    try {
      ...
      await Customer.findByIdAndUpdate(order?.customer, {
        $inc: { balance: parseInt(item.quantity) * parseFloat(item.price) },
      });
